/**
 * IncidentHub AI — Master Demo Database Seed Script
 *
 * Purpose: Populates the database with realistic, high-fidelity engineering incident data:
 * - Multi-tenant organization with RBAC members (Owner, Admin, Responder, Viewer)
 * - SRE & Core Engineering Teams, Projects, and Services
 * - Realistic Incidents across SEV-1 to SEV-4 with coherent MTTD/MTTR timestamps
 * - AI Root-Cause Investigation runs (evidence-grounded hypotheses, tokens, latencies)
 * - Comprehensive Postmortems with versions, 5-whys, and actionable items
 * - GitHub commits, pull requests, deployments, and correlated evidence
 * - Analytics snapshots for instant dashboard metrics visualization
 *
 * Usage:
...
async function main(): Promise<void> {
  console.log('🚀 Starting IncidentHub AI Demo Database Seeding...\n');

  const passwordHash = await bcrypt.hash('Password123!', 10);

  // ---------------------------------------------------------------------------
  // 1. Find Existing Users or Create Demo Users
  // ---------------------------------------------------------------------------
  console.log('👤 [1/7] Creating/Updating Demo Users & RBAC Roles...');

  // Primary demo owner (or link to existing user)
  const existingUsers = await prisma.user.findMany({ take: 5 });
  const primaryOwnerEmail = existingUsers.length > 0 ? existingUsers[0].email : 'rakesh.rajput@acme.dev';

  const demoOwner = await prisma.user.upsert({
    where: { email: primaryOwnerEmail },
    update: { name: existingUsers.length > 0 ? existingUsers[0].name : 'Rakesh Rajput' },
    create: {
  ...
  console.log(`  ✔ Responder: ${demoResponder.email}`);
  console.log(`  ✔ Viewer: ${demoViewer.email}`);

  // ---------------------------------------------------------------------------
  // 2. Organization & RBAC Memberships
  // ---------------------------------------------------------------------------
  console.log('\n🏢 [2/7] Provisioning Organization & Team Hierarchy...');
  ...
  // Assign memberships with explicit RBAC roles
  const memberships = [
    { userId: demoOwner.id, role: OrgRole.OWNER },
    { userId: demoAdmin.id, role: OrgRole.ADMIN },
    { userId: demoResponder.id, role: OrgRole.RESPONDER },
    { userId: demoViewer.id, role: OrgRole.VIEWER },
  ];

  // Also include any other existing users in DB as OWNER/ADMIN so they see the demo org immediately
  for (const u of existingUsers) {
    if (!memberships.some((m) => m.userId === u.id)) {
      memberships.push({ userId: u.id, role: OrgRole.OWNER });
    }
  }
  ...
  console.log(`  ✔ Organization: "${org.name}" (${org.slug}) with ${memberships.length} members`);

  // ---------------------------------------------------------------------------
  // 3. Teams, Projects & Services
  // ---------------------------------------------------------------------------
  ...
  console.log(`  ✔ Teams: "${sreTeam.name}", "${backendTeam.name}"`);
  console.log(`  ✔ Projects: "${paymentProject.name}", "${coreApiProject.name}"`);
  console.log(`  ✔ Services: "${paymentGatewayService.name}", "${checkoutApiService.name}", "${authService.name}"`);

  // ---------------------------------------------------------------------------
  // 4. GitHub Integration & Telemetry Repository
  // ---------------------------------------------------------------------------
  console.log('\n🐙 [3/7] Setting up GitHub Telemetry & Correlated Repository...');
  ...
  const culpritCommit = await prisma.gitHubCommit.upsert({
    ...
    create: {
      ...
      committedAt: new Date(Date.now() - 5 * 86400 * 1000 - 15 * 60 * 1000), // 15 mins before incident
    },
  });
  ...
  const culpritDeploy = await prisma.gitHubDeployment.upsert({
    ...
    create: {
      ...
      createdAt: new Date(Date.now() - 5 * 86400 * 1000 - 10 * 60 * 1000), // 10 mins before incident
    },
  });


  console.log(`  ✔ Connected GitHub Repo: "${githubRepo.fullName}"`);
  console.log(`  ✔ Correlated Commit: ${culpritCommit.sha.slice(0, 7)} — "${culpritCommit.message}"`);

  // ---------------------------------------------------------------------------
  // 5. Realistic Incidents with Coherent Timelines
  // ---------------------------------------------------------------------------
  console.log('\n🚨 [4/7] Generating Realistic Incidents Across SEV1–SEV4...');

  const now = Date.now();

  // Helper for incident dates
  const daysAgo = (d: number, hours = 0, mins = 0) => new Date(now - d * 86400000 - hours * 3600000 - mins * 60000);

  // Clean old demo incidents under Acme to ensure deterministic re-seeding
  await prisma.incident.deleteMany({ where: { organizationId: org.id } });

  // INCIDENT 1: SEV-1 RESOLVED (Full AI Investigation & Postmortem showcase)
  const inc1Detected = daysAgo(5, 4, 15);
  const inc1Ack = daysAgo(5, 4, 11);
  const inc1Resolved = daysAgo(5, 2, 52); // Duration = 83 mins
  ...
  // INCIDENT 2: SEV-2 RESOLVED (AI Investigation showcase)
  const inc2Detected = daysAgo(3, 8, 30);
  const inc2Ack = daysAgo(3, 8, 26);
  const inc2Resolved = daysAgo(3, 7, 16); // Duration = 74 mins
  ...
  const inc2 = await prisma.incident.create({
    data: {
      ...
      acknowledgedAt: inc2Ack,
      resolvedAt: inc2Resolved,
  ...
  // INCIDENT 3: SEV-2 MITIGATING (Active incident)
  const inc3Detected = daysAgo(0, 5, 20);
  const inc3 = await prisma.incident.create({
    data: {
      ...
      environment: IncidentEnvironment.PRODUCTION,
      detectedAt: inc3Detected,
      acknowledgedAt: daysAgo(0, 5, 12),
      createdById: demoResponder.id,
      assignedToId: demoResponder.id,
      createdAt: inc3Detected,
    },
  });

  // INCIDENT 4: SEV-3 INVESTIGATING
  const inc4Detected = daysAgo(0, 2, 45);
  const inc4 = await prisma.incident.create({
    data: {
      ...
      environment: IncidentEnvironment.PRODUCTION,
      detectedAt: inc4Detected,
      acknowledgedAt: daysAgo(0, 2, 35),
      createdById: demoAdmin.id,
      assignedToId: demoAdmin.id,
      createdAt: inc4Detected,
    },
  });

  // INCIDENT 5: SEV-4 OPEN
  const inc5Detected = daysAgo(0, 0, 45);
  const inc5 = await prisma.incident.create({
    data: {
      ...
      detectedAt: inc5Detected,
      createdById: demoViewer.id,
      createdAt: inc5Detected,
    },
  });

  // INCIDENT 6: SEV-3 RESOLVED
  const inc6Detected = daysAgo(9, 6, 0);
  const inc6 = await prisma.incident.create({
    data: {
      ...
      status: IncidentStatus.RESOLVED,
      environment: IncidentEnvironment.PRODUCTION,
      detectedAt: inc6Detected,
      acknowledgedAt: daysAgo(9, 5, 55),
      resolvedAt: daysAgo(9, 4, 10), // 110 mins
      createdById: demoAdmin.id,
      assignedToId: demoResponder.id,
      createdAt: inc6Detected,
    },
  });

  // INCIDENT 7: SEV-4 RESOLVED
  const inc7Detected = daysAgo(14, 12, 0);
  const inc7 = await prisma.incident.create({
    data: {
      ...
      status: IncidentStatus.RESOLVED,
      environment: IncidentEnvironment.PRODUCTION,
      detectedAt: inc7Detected,
      acknowledgedAt: daysAgo(14, 11, 58),
      resolvedAt: daysAgo(14, 11, 20), // 40 mins
      createdById: demoAdmin.id,
      assignedToId: demoAdmin.id,
      createdAt: inc7Detected,
    },
  });

  console.log(`  ✔ Created 7 Incidents (SEV1: 1, SEV2: 2, SEV3: 2, SEV4: 2)`);

  // ---------------------------------------------------------------------------
  // 6. Evidence, Timeline Events, Comments & Replay for Incident 1
  // ---------------------------------------------------------------------------
  console.log('\n🔍 [5/7] Linking Telemetry Evidence, Timeline & Discussion Comments...');

  // Incident 1 Correlation Run
  ...
  const evidence1 = await prisma.incidentEvidence.create({
    data: {
      ...
      scoreBreakdown: { temporal: 0.35, topology: 0.35, deployment: 0.24 },
    },
  });
  ...
  const evidence3 = await prisma.incidentEvidence.create({
    data: {
      incidentId: inc1.id,
      correlationRunId: corrRun1.id,
  ...
  const rootComment = await prisma.comment.create({
    data: {
      ...
      userId: demoAdmin.id,
      content:
        'Seeing 504 Gateway Timeouts surge across all US-East payment pods. Postgres pg_stat_activity shows 200/200 active connections in idle in transaction state.',
      createdAt: new Date(inc1Detected.getTime() + 8 * 60000),
    },
  });
  ...
  await prisma.comment.create({
    data: {
      ...
      content:
        'Found it! PR #412 reduced idle connection timeout to 200ms. High frequency health check probes are thrashing the pool creating connection storm. Rolling back PR #412 now.',
      createdAt: new Date(inc1Detected.getTime() + 25 * 60000),
    },
  });

  // Replay Run for Incident 1
  ...
  for (const re of replayEventsData) {
    await prisma.replayEvent.create({
      data: {
        ...
        sourceEventId: `event:${re.sequenceIndex}:${re.eventType}`,
  ...
  console.log(`  ✔ Incident 1: 3 Telemetry Evidence items, 5 Timeline Events, 2 Comments, 1 Replay Run`);

  // ---------------------------------------------------------------------------
  // 7. AI Investigation Engine & Grounded Postmortems (Incidents 1 & 2)
  // ---------------------------------------------------------------------------
  console.log('\n🤖 [6/7] Synthesizing AI Root-Cause Investigations & Postmortems...');

  // Incident 1 AI Investigation Run
  await prisma.investigationRun.create({
    data: {
      ...
      totalTokens: 2460,
      latencyMs: 1480,
      startedAt: new Date(inc1Detected.getTime() + 10 * 60000),
      completedAt: new Date(inc1Detected.getTime() + 12 * 60000),
    },
  });

  // Incident 1 Postmortem & Postmortem Version
  ...
  const pmVersion1 = await prisma.postmortemVersion.create({
    data: {
      ...
      evidenceReferences: {
        deploymentId: culpritDeploy.deploymentId,
        commitSha: culpritCommit.sha,
        pullRequestNumber: culpritPR.number,
      },
      providerName: 'openai',
      modelName: 'gpt-4o',
      promptTokens: 2150,
  ...
  // Incident 2 AI Investigation Run
  await prisma.investigationRun.create({
    data: {
      organizationId: org.id,
      incidentId: inc2.id,
      ...
      confidence: 0.93,
      incidentSummary:
        'Auth & Session Service experienced a 30x latency degradation on /api/v1/auth/verify following deployment v2.14.0 due to an un-cached JWT public key verification loop.',
      probableRootCause:
        'Auth service v2.14.0 introduced key rotation validation but omitted the Redis cache layer for RS256 public keys, forcing cryptographic key reconstruction on every incoming API request.',
      impactAssessment: 'Affected 88,000 API requests. Increased API Gateway p99 from 45ms to 1,420ms.',
      recommendedActions: [
        'Restore Redis caching for public keys with 1-hour TTL and jitter.',
        'Add p99 latency regression assertion to staging automated suite.',
      ],
      providerName: 'openai',
      modelName: 'gpt-4o',
      promptTokens: 1650,
      ...
      totalTokens: 2130,
      latencyMs: 1320,
      startedAt: new Date(inc2Detected.getTime() + 10 * 60000),
      completedAt: new Date(inc2Detected.getTime() + 11 * 60000),
    },
  });

  console.log(`  ✔ Synthesized AI Investigation runs & Postmortems for Incidents #101 and #102`);

  // ---------------------------------------------------------------------------
  // 8. Derived Analytics Snapshots (for instant metric charts)
  // ---------------------------------------------------------------------------
  console.log('\n📊 [7/7] Generating Derived Analytics Snapshots (MTTR, MTTD, CFR)...');
  ...
  await prisma.analyticsSnapshot.upsert({
    ...
    update: {
      ...
      sev3Count: 2,
      sev4Count: 2,
      mttdMs: 4.2 * 60 * 1000, // 4.2 mins MTTD
      mttrMs: 78.5 * 60 * 1000, // 78.5 mins MTTR
      cfrPercent: 12.5,
    },
    create: {
      organizationId: org.id,
  ...
  console.log(`  ✔ Seeded 30d & 7d Analytics Snapshots (MTTD: 4.2m, MTTR: 78.5m, CFR: 12.5%)`);
  ...
  const summary = {
    ...
    members: await prisma.organizationMember.count({ where: { organizationId: org.id } }),
    teams: await prisma.team.count({ where: { organizationId: org.id } }),
    projects: await prisma.project.count({ where: { organizationId: org.id } }),
    services: await prisma.service.count({ where: { project: { organizationId: org.id } } }),
    incidents: await prisma.incident.count({ where: { organizationId: org.id } }),
    evidence: await prisma.incidentEvidence.count({ where: { incident: { organizationId: org.id } } }),
    investigations: await prisma.investigationRun.count({ where: { organizationId: org.id } }),
    postmortems: await prisma.postmortem.count({ where: { organizationId: org.id } }),
    actionItems: await prisma.actionItem.count({ where: { organizationId: org.id } }),
  };

  console.log('\n=============================================================');
  console.log('🎉 IncidentHub AI Demo Database Seeding Completed!');
  console.log('=============================================================');
  console.table(summary);
  console.log('\nDemo User Logins (Password: "Password123!"):');
  console.log(`- Owner:     ${demoOwner.email}`);
  console.log(`- Admin:     ${demoAdmin.email}`);
  console.log(`- Responder: ${demoResponder.email}`);
  console.log(`- Viewer:    ${demoViewer.email}`);
